...
const tshirts = ({products}) => {
  ...
  <div className="container px-5 py-24 mx-auto">
    ...
    {Object.keys(products).map((item)=>{
     return <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-lg ">
          <Link className="block relative h-48 rounded overflow-hidden" href={`/product/${products[item].slug}`}><img alt="ecommerce" className="object-contain object-center w-full h-full block" src={products[item].img}/></Link>
          
        <div className="mt-4">
          <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">{products[item].category}</h3>
          <h2 className="text-gray-900 title-font text-lg font-medium">{products[item].title}</h2>
          <p className="mt-1">Rs{products[item].price}</p>
          <Link  href={`/product/${products[item].slug}`}><button className='mt-1 bg-green-500 p-2 text-white my-1'>Buy Now</button></Link>
        </div>
        </div>
    })}
   </div>
